...
const SELFIE_INSTRUCTIONS = [
  "Ensure you're in a bright environment or enough lighting in your background",
  "Avoid wearables such as eye glasses, mask etc",
  "Ensure to hold still while your face is being scanned.",
  "Ensure you're in a bright environment",
];
...
export default function KYCView() {
  ...
  const [verifyLiveness, { isLoading: isVerifyingLiveness }] =
    useVerifyLivenessMutation();
  ...
  const handleBack = () => {
    ...
    else if (step === "selfie-instructions") setStep("id-input");
    else if (step === "capture") setStep("selfie-instructions");
    else if (step === "liveness") setStep("capture");
  };
  ...
  const handleNext = async () => {
    ...
    } else if (step === "selfie-instructions") {
      setCaptureOpen(true);
    }
  };
  ...
  const handleLivenessVerification = async () => {
    ...
    try {
      const result = await verifyLiveness({ photo: capturedPhoto }).unwrap();
      if (result.is_real) {
        setStep("success");
      } else {
        toast.error(
          `Liveness check failed (score: ${result.score}). Please try again.`
        );
        setStep("capture");
      }
    } catch (err) {
      ...
      if (error.status === 503) {
        ...
        toast.error(
          error?.data?.message ?? "Liveness check failed. Please try again."
        );
      }
      setStep("capture");
    }
  ...
  useEffect(() => {
    if (step === "liveness" && capturedPhoto) {
      handleLivenessVerification();
    }
  ...
  return (
    <FormProvider {...methods}>
      <form
        ...
        className="flex flex-col min-h-screen animate-in fade-in duration-500 w-full relative"
      >
        {/* Top Navigation Bar / Header */}
        {step !== "success" && step !== "liveness" && (
          <header className="h-[60px] bg-white border-b border-[#F0F0F0] flex items-center px-[24px] sticky top-0 z-30 justify-between w-full shrink-0">
            <div className="flex items-center gap-[16px]">
              <button
                ...
                className="flex items-center gap-[8px] text-[#202020] hover:text-[#0063EF] transition-colors font-medium text-[14px] disabled:opacity-50"
              >
        ...
        {step === "personal" && (
          <div className="flex flex-col items-center flex-1 max-w-[500px] mx-auto w-full py-[60px] px-[20px]">
            <div className="flex flex-col items-center text-center w-full mb-[40px]">
              <h1 className="text-[32px] font-semibold text-[#202020] tracking-[-0.64px] leading-[40px] mb-[16px]">
                Personal details
              </h1>
        ...
        {step === "id-selection" && (
          <div className="flex flex-col items-center flex-1 max-w-[500px] mx-auto w-full py-[60px] px-[20px]">
            <div className="flex flex-col items-center text-center w-full mb-[40px]">
              ...
              <p className="text-[16px] text-[#606060] leading-[24px]">
                Select a recognized government ID to continue
              </p>
            </div>
            ...
            <div className="w-full flex flex-col gap-[40px]">
              <div className="flex flex-col gap-[16px]">
                ...
                <div className="flex flex-col gap-[16px]">
                  <div className="flex flex-col gap-[8px]">
                    <span className="text-[14px] font-medium text-[#202020]">
                      Select ID Type{" "}
                      <span className="text-[#FF5025]">*</span>
                    </span>
                    {ID_OPTIONS.map((type) => (
                      <div
                        key={type}
                        onClick={() =>
                          setValue("selectedId", type, {
                            shouldValidate: true,
                          })
                        }
                        className={cn(
                    ...
                    {errors.selectedId && (
                      <p className="text-caption-xs text-[#FF5025] mt-[4px]">
              ...
              <div className="flex flex-col gap-[56px]">
                <p className="text-[14px] text-[#606060] leading-[20px]">
                  By clicking continue, you agree to SoluDesks Verification
        ...
        {step === "id-input" && (
          <div className="flex flex-col items-center flex-1 max-w-[500px] mx-auto w-full py-[60px] px-[20px]">
            <div className="flex flex-col items-center text-center w-full mb-[40px]">
              ...
              <p className="text-[16px] text-[#606060] leading-[24px]">
                Please enter your {selectedId?.toLowerCase()} number in the
                field below
              </p>
            </div>
        ...
        {/* Step 4: Selfie Instructions */}
        {step === "selfie-instructions" && (
          <div className="flex flex-col items-center flex-1 max-w-[500px] mx-auto w-full py-[60px] px-[20px]">
            <div className="flex flex-col items-center text-center w-full mb-[40px]">
              <h1 className="text-[32px] font-bold text-[#202020] leading-[40px] mb-[16px] font-[Quicksand]">
                Selfie verification
              </h1>
              <p className="text-[16px] text-[#636363] tracking-[-0.32px] leading-[24px]">
                Please follow the instructions below to complete your facial
                verification
              </p>
            </div>
            ...
            <div className="w-full flex flex-col items-center gap-[40px]">
              {/* Face scan icon */}
              <Image
                src="/images/kyc/face-scan-icon.png"
                alt="Face scan"
                width={91}
                height={91}
              ...
              <div className="w-full flex flex-col gap-[8px]">
                {SELFIE_INSTRUCTIONS.map((instruction, idx) => (
                  ...
                  >
                    <Image
                      src="/images/kyc/bullet-dot.svg"
                      alt=""
                      width={6}
                      height={6}
                      className="mt-[5px] shrink-0"
                    />
                    <span className="text-[12px] text-[#606060] leading-[16px]">
                      {instruction}
                    </span>
                  </div>
                ))}
              ...
              <div className="flex flex-col items-center gap-[16px] w-full">
                <p className="text-[14px] text-[rgba(0,0,0,0.7)] text-center tracking-[-0.28px] leading-[20px]">
                  By clicking continue, you agree to{" "}
                  <a
                    href="https://www.soludesk.com"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="underline"
                  >
                    Soludesk Verification Policy
                  </a>{" "}
                  and{" "}
                  <a
                    href="https://soludesk.com"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="underline"
                  >
        ...
        {step === "liveness" && (
          <div className="flex flex-col items-center justify-center flex-1 max-w-[500px] mx-auto w-full py-[60px] px-[20px]">
            <div className="flex flex-col items-center gap-[24px]">
              <div className="h-10 w-10 animate-spin rounded-full border-4 border-[#E8E8E8] border-t-[#0063EF]" />
              <div className="flex flex-col items-center gap-[8px]">
                <h2 className="text-[20px] font-semibold text-[#202020] leading-[28px]">
                  Verifying liveness
                </h2>
                <p className="text-[14px] text-[#606060] text-center">
